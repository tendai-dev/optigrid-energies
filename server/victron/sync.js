...
// Config
// ---------------------------------------------------------------------------
const FEED_URL = process.env.VICTRON_FEED_URL || 'https://www.victronenergy.com/api/v1/products?format=xml';
const FETCH_TIMEOUT = parseInt(process.env.VICTRON_FETCH_TIMEOUT_MS, 10) || 30000;
...
function fetchXml(url) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(FETCH_TIMEOUT, () => {
      req.destroy();
      reject(new Error(`Victron feed request timed out after ${FETCH_TIMEOUT}ms`));
    });
  });
}
...
function upsertCategory(db, name) {
  ...
  const existing = db.prepare('SELECT id FROM victron_categories WHERE slug = ?').get(slug);
  if (existing) return existing.id;

  const result = db.prepare(
    'INSERT INTO victron_categories (name, slug) VALUES (?, ?)'
  ).run(name, slug);
...
function upsertProduct(db, product, categoryId) {
  ...
  const slugCollision = db.prepare(
  ...
  if (existing) {
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run(
      slug, product.name, product.short_name, product.model, product.sku,
      categoryId, product.short_description, product.full_description,
      product.product_url, product.image_url, product.modified_at,
      product.raw_payload, existing.id
    );
    ...
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
    `).run(
      product.external_id, slug, product.name, product.short_name,
      product.model, product.sku, categoryId, product.short_description,
      product.full_description, product.product_url, product.image_url,
      product.modified_at, product.raw_payload
    );
  ...
  db.prepare('DELETE FROM victron_product_documents WHERE product_id = ?').run(productId);
  ...
  for (let i = 0; i < product.documents.length; i++) {
    ...
    insertDoc.run(
      productId, doc.external_id, doc.type, doc.title, doc.url,
      doc.file_format, i, doc.metadata_json
    );
  }
...
async function syncVictronProducts() {
  ...
  const result = { total: 0, inserted: 0, updated: 0, skipped: 0, failed: 0, errors: [] };
  ...
  console.log(`[victron-sync] Parsing XML (${(xml.length / 1024).toFixed(0)} KB)`);
  let products;
  try {
  ...
  console.log(`[victron-sync] Parsed ${products.length} products. Upserting...`);
