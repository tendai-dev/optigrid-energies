...
(function () {
  ...
  function renderCard(product) {
    const badges = (product.asset_types || [])
      ...
      .map(
        (t) =>
          `<span class="vc-badge vc-badge--${t}">${ASSET_LABELS[t]}</span>`,
      )
      .join("");

    const imgSrc =
      product.image_url || "https://via.placeholder.com/400x260?text=No+Image";
  ...
  pagination.addEventListener("click", (e) => {
    ...
    load();
    window.scrollTo({
      top: document.querySelector(".vc-toolbar").offsetTop - 80,
      behavior: "smooth",
    });
  });
